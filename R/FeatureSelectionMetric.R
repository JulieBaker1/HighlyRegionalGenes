...
PCA10_explanation <- function(obj,gene_all_methods){
  a=parallel::detectCores(logical = F)
  if(a<7){
    a=a-1
  }else{
    a=7
  }
  cl <- parallel::makeCluster(getOption("cl.cores", a))
  methods_name=colnames(gene_all_methods)
  all.genes=rownames(obj)
  obj=ScaleData(obj,features = all.genes)
  scale.data=GetAssayData(obj,slot="scale.data")

  parallel::clusterExport(cl=cl, varlist=c("scale.data","gene_all_methods"), envir=environment())

  top10_explanation <- parallel::parLapply(cl,1:length(methods_name),PCA_SD)
...
gene_correlation <- function(obj,gene_all_methods){
  ...
  cor_cos <- parallel::parLapply(cl,1:length(methods_name),correlation_cal)
